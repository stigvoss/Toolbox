...
namespace Toolbox.Validation
{
    public static class Ensure
    {
        ...
        public static void IsZeroOrBelow<TException>(this float subject, string message = null)
            where TException : Exception
        {
            IsZeroOrBelow<TException>((double)subject);
        }
            ...
            string message)
        {
            ...
            if (rejectCondition(subject))
            {
                if (message == null)
                {
                    exception = ExceptionFrom(type);
                }
                else
                {
                    exception = ExceptionWithMessageFrom(type, message);
                }
        ...
        private static Exception ExceptionFrom(
            Type type)
        {
            return (Exception)Activator.CreateInstance(type);
        }

        private static Exception ExceptionWithMessageFrom(
            Type type,
            string message)
        {
